<script lang="ts">
	...
	import { getCitationUrl, isHttpUrl, normalizeCitationSource } from './citations';
	...
	$: {
		citations = sources.reduce((acc, source) => {
			...
			source?.document?.forEach((document, index) => {
				...
				const _source = normalizeCitationSource(source, metadata, id);
...
{#if citations.length > 0}
	{@const urlCitations = citations.filter((c) => isHttpUrl(getCitationUrl(c)))}
			...
			{#if urlCitations.length > 0}
				<div class="flex -space-x-1 items-center">
					{#each urlCitations.slice(0, 3) as citation, idx}
						<img
							src="https://www.google.com/s2/favicons?sz=32&domain={getCitationUrl(citation)}"
							alt="favicon"
