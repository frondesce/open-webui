...
def convert_output_to_messages(
    ...
    Args:
        ...
        reasoning_format: How to include reasoning blocks in the output:
            ...
            - ``'reasoning_content'``: set as ``reasoning_content`` top-level field
              (for llama.cpp and models that need native reasoning replay).
    ...
    completed_call_ids = {
        ...
        and (item.get('status') is None or item.get('status') in {'completed', 'failed', 'rejected'})
    }
    result_call_ids = {
    ...
    async def yield_safe_stream_chunks():
        ...
        async for data, _ in stream.iter_chunks():
                ...
                *lines, rest = bytes(buffer).split(b'\n')
                ...
                for line in lines:
                    if len(line) > max_buffer_size:
                        log.info('Replacing line over max buffer size: %s bytes', len(line))
                        yield b'data: {}\n'
                    else:
                        yield line + b'\n'
            ...
            if len(buffer) > max_buffer_size:
                log.info('Replacing line over max buffer size, buffered so far: %s bytes', len(buffer))
